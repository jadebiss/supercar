...
<!-- Affichage des voitures par recherche -->
...
if (isset($_POST['search_query'])) {
    ...
    $requete = "SELECT idvoiture, marque, modele, image, description1 FROM voitures WHERE marque LIKE '%$searchQuery%' OR modele LIKE '%$searchQuery%'";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            // Nouvelle ligne apres l'affichage de 4 voitures
            if ($count % 4 == 0) {
            ...
            echo '</div>';
            $count++;
            // Finir la ligne apres 4 voitures
            if ($count % 4 == 0) {
                echo '</div>';
            }
        }
        // Si la ligne ne contient que 1 a 3 elements : finir
        if ($count % 4 != 0) {
            echo '</div>';
        }
    } else {
